...
def _logins_enabled(conn) -> bool:
    # honour global switch if present; default = enabled
    val = _get_setting(conn, "logins_enabled", "1")
    return str(val) == "1"
...
def login():
    if request.method == "POST":
        try:
            conn = get_db()
            try:
                # Global kill-switch for logins
                if not _logins_enabled(conn):
                    flash("Logins are currently disabled. Please try again later.", "error")
