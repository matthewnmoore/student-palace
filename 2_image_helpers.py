...
# Target canvas (consistent across all outputs)
TARGET_W = 1600
TARGET_H = 900  # 16:9
WATERMARK_TEXT = os.environ.get("WATERMARK_TEXT", "Student Palace")
# Brand-light purple from your CSS: --brand-light:#7D3FC6
PADDING_HEX = os.environ.get("PADDING_HEX", "#7D3FC6")
...
def _hex_to_rgb(h: str) -> Tuple[int, int, int]:
    ...
    try:
        ...
        return (125, 63, 198)  # fallback to #7D3FC6
...
def _fit_and_pad_to_canvas(im: Image.Image, target_w: int, target_h: int, bg_rgb: Tuple[int,int,int]) -> Tuple[Image.Image, Tuple[int,int,int,int]]:
    """
    Resize with aspect ratio to fit within target_w x target_h.
    Center on a solid background of bg_rgb. Return (canvas, content_bbox),
    where content_bbox = (x, y, w, h) is the pasted image box on the canvas.
    """
    src_w, src_h = im.size
...
def _load_font_for_width(img_width: int) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
    # Scale watermark ~6–8% of the *visible image area* width
...
def _watermark_on_canvas(canvas: Image.Image, content_bbox: Tuple[int,int,int,int], text: str) -> Image.Image:
    """
    Draw watermark anchored to the top-left corner of the *content area*,
    not the padding bars. Keeps it consistent even with letterboxing.
    """
    ...
    font = _load_font_for_width(cw)

    # Padding offset relative to content area
    pad = max(12, cw // 80)
    x = cx + pad
    y = cy + pad
...
def process_image(buf: bytes) -> Image.Image:
    """
    Open → orient → fit to 1600x900 canvas with brand-light purple padding →
    watermark anchored to visible image area (top-left).
    """
    im = open_image_safely(buf)
    bg = _hex_to_rgb(PADDING_HEX)
    canvas, bbox = _fit_and_pad_to_canvas(im, TARGET_W, TARGET_H, bg)
    stamped = _watermark_on_canvas(canvas, bbox, WATERMARK_TEXT)
    return stamped
