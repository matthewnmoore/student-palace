...
def watermark(im: Image.Image, text: str = WATERMARK_TEXT) -> Image.Image:
    # Fallback if env var missing/empty
    ...
    bg_pad_x = max(6, pad // 2)
    bg_pad_y = max(4, pad // 3)
    rx1, ry1 = x - bg_pad_x, y - bg_pad_y
    rx2, ry2 = x + tw + bg_pad_x, y + th + bg_pad_y
    try:
        draw.rounded_rectangle([rx1, ry1, rx2, ry2], radius=6, fill=(0, 0, 0, 90))
    except Exception:
        draw.rectangle([rx1, ry1, rx2, ry2], fill=(0, 0, 0, 90))
    ...
    try:
        logger.info(f"[WM] applied size={w}x{h} text_len={len(text)}")
    except Exception:
        pass
