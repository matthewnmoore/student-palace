...
# Reuse the proven house-photo pipeline
from image_helpers import (
    logger,                    # same logger
    process_image,             # open → resize → watermark
...
def assert_room_images_schema(conn) -> None:
    ...
    """)
    # Add-only guards (handles legacy)
...
def _count_for_room(conn, rid: int) -> int:
    return int(conn.execute("SELECT COUNT(*) AS c FROM room_images WHERE room_id=?", (rid,)).fetchone()["c"])
...
def _ensure_primary_flag_room(conn, rid: int) -> int:
    r = conn.execute("SELECT COUNT(*) AS c FROM room_images WHERE room_id=? AND is_primary=1", (rid,)).fetchone()
...
def accept_upload_room(conn, rid: int, file_storage, *, enforce_limit: bool = True) -> Tuple[bool, str]:
    """
    Returns (ok, message). Saves processed JPEG to disk + DB row in room_images.
    """
    ...
    try:
        im = process_image(data)  # same pipeline (EXIF → resize 1600 → watermark)
    except Exception:
    ...
    logger.info(
        f"[UPLOAD-RM] room={rid} name={original_name!r} saved={fname!r} "
        f"size_bytes={byt} dims={w}x{h}"
    )
