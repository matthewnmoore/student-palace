...
def _room_form_values(request):
    ...
    from utils import clean_bool as _clean_bool
    ensuite = _clean_bool("ensuite")
    bed_size = (request.form.get("bed_size") or "").strip()
    tv = _clean_bool("tv")
    desk_chair = _clean_bool("desk_chair")
    wardrobe = _clean_bool("wardrobe")
    chest_drawers = _clean_bool("chest_drawers")
    lockable_door = _clean_bool("lockable_door")
    wired_internet = _clean_bool("wired_internet")
...
# -------- Rooms CRUD --------
@landlord_bp.route("/landlord/houses/<int:hid>/rooms", endpoint="rooms_list")
def rooms_list(hid):
    r = require_landlord()
    if r: return r
    lid = current_landlord_id()
