    ...
    if request.method == "POST":
        ...
        if errors:
            ...
            return render_template(
                ...
                rooms_configured=0,  # ensure template var always exists
            )
...
def house_edit(hid):
    ...
    # NEW: how many rooms already configured for this house?
    try:
        rc_row = conn.execute("SELECT COUNT(*) AS c FROM rooms WHERE house_id=?", (hid,)).fetchone()
        rooms_configured = int(rc_row["c"]) if rc_row else 0
    except Exception:
        rooms_configured = 0
    ...
    if request.method == "POST":
        ...
        if errors:
            ...
            return render_template(
                ...
                house=house,
                default_listing_type=default_listing_type,
                rooms_configured=rooms_configured,  # ensure template var exists on error re-render
            )
    ...
    return render_template(
        ...
        house=house,
        default_listing_type=default_listing_type,
        rooms_configured=rooms_configured,  # supply to template on GET
    )
