...
def _parse_or_delegate(form, mode: str, default_listing_type: str,
    ...
    if conn is not None:
        ...
        if city_raw and pc_prefix:
            ...
            if not row:
                errors.append(f"Postcode {pc_prefix} is not valid for {city_raw}.")
...
def house_new():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if new_hid: recompute_house_summaries(conn,new_hid)
...
def house_edit(hid):
    ...
    if request.method == "POST":
        ...
        try: recompute_house_summaries(conn,hid)
        except Exception: pass
