...
def house_edit(hid):
    ...
    # --- NEW: count rooms for this house (used only on Edit page) ---
    try:
        rc = conn.execute("SELECT COUNT(*) AS c FROM rooms WHERE house_id=?", (hid,)).fetchone()
        rooms_count = int(rc["c"]) if rc is not None else 0
    except Exception:
        rooms_count = 0
    ...
    if request.method == "POST":
        ...
        if errors:
            ...
            return render_template(
                ...
                house=house,
                default_listing_type=default_listing_type,
                rooms_count=rooms_count,   # NEW: ensure POST re-render shows the count
            )

        # 🔒 Safeguard: ensure bedrooms_total is never below configured rooms
    ...
    return render_template(
        ...
        house=house,
        default_listing_type=default_listing_type,
        rooms_count=rooms_count,  # NEW: supply to template on GET
    )
