...
def _parse_or_delegate(form, mode: str, default_listing_type: str, existing_address: str | None = None):
    ...
    if bills_option == "yes":
        bills_util = dict(bills_util_gas=1, bills_util_electric=1, bills_util_water=1,
        ...
        bills_util = dict(bills_util_gas=0, bills_util_electric=0, bills_util_water=0,
    ...
    if request.method == "POST":
        ...
        if errors:
            for e in errors:
                flash(e, "error")
            conn.close()
            return render_template("house_form.html", cities=cities, form=request.form, mode="new", default_listing_type=default_listing_type)
...
def house_edit(hid):
    ...
    if request.method == "POST":
        payload, errors = _parse_or_delegate(request.form, mode="edit", default_listing_type=default_listing_type, existing_address=house.get("address"))
        if errors:
            for e in errors:
                flash(e, "error")
            conn.close()
            return render_template("house_form.html", cities=cities, form=request.form, mode="edit", house=house, default_listing_type=default_listing_type)
    ...
    # ✅ NEW: count rooms
    rooms_count = conn.execute("SELECT COUNT(*) AS c FROM rooms WHERE house_id=?", (hid,)).fetchone()["c"]

    conn.close()
    return render_template("house_form.html", cities=cities, form=form, mode="edit", house=house, rooms_count=rooms_count, default_listing_type=default_listing_type)
