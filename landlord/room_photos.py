...
# ---------------- Debug Route ----------------
@bp.route("/landlord/houses/<int:hid>/rooms/<int:rid>/photos/debug")
def room_photos_debug(hid: int, rid: int):
    r = require_landlord()
    if r: return r
    lid = current_landlord_id()
    conn = get_db()
    ...
    for r in rows:
        ...
        out.append({
            ...
            "exists_on_disk": os.path.exists(file_abs_path_room(fname)),
            "is_primary": int(r["is_primary"]) == 1,
            "size_wxh": f'{r["width"]}x{r["height"]}',
            "bytes": r["bytes"],
        })
    return {"room_id": rid, "items": out}, 200, {"Content-Type": "application/json"}
