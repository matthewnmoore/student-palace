...
<section class="wrap">
  ...
  <form method="post" class="form-stacked">
    ...
    <div class="card card--accent-lr">
      ...
      <!-- Row 1: Room name, Price per month, Price per week (readonly), Approx. room size -->
      ...
      <!-- Row 2: Bed size + checkboxes inline -->
    ...
    <div class="card card--accent-lr">
      ...
      <div class="grid grid-3" style="align-items:end">
        <label class="check" style="align-self:end">
          <input type="checkbox" name="is_let" id="is_let"
        ...
        <label class="field" id="row_let_until" style="display:none">
          ...
          <input type="date" name="let_until" id="let_until"
                 value="{{ let_until_val }}"
                 style="padding:12px; font-size:16px; min-height:44px; display:inline-block; width:100%">
        </label>
    ...
    <div class="card card--accent-lr">
      ...
      <div class="grid grid-3">
        ...
        <label class="field checkbox" style="align-self:end">
          <input type="checkbox" name="safe" {% if form and form.safe in ['1', 1, 'on', True] %}checked{% endif %}>
          <span>Safe</span>
        </label>

        <label class="field checkbox" style="align-self:end">
          <input type="checkbox" name="dressing_table" {% if form and form.dressing_table in ['1', 1, 'on', True] %}checked{% endif %}>
          <span>Dressing table</span>
        </label>

        <label class="field checkbox" style="align-self:end">
          <input type="checkbox" name="mirror" {% if form and form.mirror in ['1', 1, 'on', True] %}checked{% endif %}>
          <span>Mirror</span>
        </label>

        <label class="field checkbox" style="align-self:end">
          <input type="checkbox" name="bedside_table" {% if form and form.bedside_table in ['1', 1, 'on', True] %}checked{% endif %}>
          <span>Bedside table</span>
        </label>

        <label class="field checkbox" style="align-self:end">
          <input type="checkbox" name="blinds" {% if form and form.blinds in ['1', 1, 'on', True] %}checked{% endif %}>
          <span>Blinds</span>
        </label>

        <label class="field checkbox" style="align-self:end">
          <input type="checkbox" name="curtains" {% if form and form.curtains in ['1', 1, 'on', True] %}checked{% endif %}>
          <span>Curtains</span>
        </label>

        <label class="field checkbox" style="align-self:end">
          <input type="checkbox" name="sofa" {% if form and form.sofa in ['1', 1, 'on', True] %}checked{% endif %}>
          <span>Sofa</span>
        </label>
      </div>
    </div>
...
(function(){
  ...
  var rowAvailFrom = document.getElementById('row_available_from');
  ...
  function syncVisibility(){
    ...
    if (!checked && letUntil) letUntil.value = '';
  }
  ...
  if (isLet) {
    isLet.addEventListener('change', syncVisibility);
  }
  syncVisibility();
  wireAutoNextDay();
  ...
  if (pcmInput){
    pcmInput.addEventListener('input', updatePPW);
    updatePPW();
  }
})();
</script>
