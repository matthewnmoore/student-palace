...
# -------------------------------
# TEMP: run migration for house features (remove after running once)
# -------------------------------
try:
    from admin.migrate_add_house_features import run as migrate_features
except Exception:
    migrate_features = None  # if the file isn't present, keep the app booting

@bp.route("/run_migration_features")
def run_migration_features():
    # Admin gate
    ...
    if migrate_features is None:
        return "<pre>Migration script not found: admin/migrate_add_house_features.py</pre>", 500

    result = migrate_features()
