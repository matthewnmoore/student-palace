...
# ======== ADD-ONLY: disabled rollups (no edits to existing code) ========

def _ensure_disabled_rollup_columns(conn: Connection) -> None:
    """
    Ensure new disabled-related rollup columns exist on houses (idempotent).
    """
...
def _iter_rooms_disabled(conn: Connection, house_id: int) -> Iterable[dict]:
    """
    Minimal room fetch that includes disabled_ok + is_let only.
    """
    return conn.execute(
        """
        SELECT
...
def recompute_house_summaries_disabled(conn: Connection, house_id: int) -> dict:
    """
    Compute and persist disabled-related rollups ONLY, without changing any
    existing summary logic. Safe to call alongside your existing
    recompute_house_summaries().
    """
    _ensure_disabled_rollup_columns(conn)

    rows = list(_iter_rooms_disabled(conn, house_id))
    suitable_for_disabled_total = 0
    disabled_ok_available = 0

    for r in rows:
        is_disabled = int(r["disabled_ok"]) == 1
        is_available = int(r["is_let"]) == 0
        if is_disabled:
    ...
    conn.execute(
        """
        UPDATE houses
           SET suitable_for_disabled_total = ?,
               disabled_ok_available       = ?
         WHERE id = ?
        """,
        (suitable_for_disabled_total, disabled_ok_available, house_id)
    )
    conn.commit()
...
def recompute_all_houses_disabled(conn: Connection) -> int:
    """
    Batch recompute disabled-related rollups for every house.
    """
    _ensure_disabled_rollup_columns(conn)
    house_ids = [row[0] for row in conn.execute("SELECT id FROM houses").fetchall()]
    for hid in house_ids:
